...
impl FromRequest for LoggedUser {
    ...
    fn from_request(req: &HttpRequest, payload: &mut dev::Payload) -> Self::Future {
        // if rand::random() {
        //     ok(LoggedUser { email: "nhitran@gmail.com".into(), company: "Demo".into() })
        // } else {
        //     err(HttpResponse::NotFound().json("no luck".to_string()))
        // }
        let csrf_token =
            req
                .headers()
        ...
        let token = csrf_token.to_str().unwrap();
        validate_signature_jwt_rs256(&token);

        //let decoded_token = decode_jwt_rs256(token).unwrap();
        //let user_identity = Identity::from_request(req, payload);
        //let token_kid = create_token_rs256(&user_identity.email, &user_identity.company);



        if let Some(identity) = "nhi".identity() {
